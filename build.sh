...
export GCC_PREFIX="arm-linaro-linux-androideabi-"
export TC_PATH="${HOME}/build/mocha/gcc-prebuilts/bin/"
...
make_a_fucking_defconfig() 
{
	...
	make -s O=$objdir $CONFIG_FILE -j8
}
compile() 
{
	echo -e ${LGR} "############### Compiling kernel ################${NC}"
	make CROSS_COMPILE=${TC_PATH}/${GCC_PREFIX} O=$objdir -j8 \
	zImage
}
ramdisk() 
{
